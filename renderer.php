...
class mod_lips_renderer extends plugin_renderer_base {
    ...
    public function display_profile_menu($current) {
        ...
        $menu .= ($action == 'solved_problems') ? '<li><p class="current">' . get_string('solved_problems', 'lips') . '</p></li>' : '<li><a href="view.php?id=' . $id . '&amp;view=' . $view . '&amp;action=solved_problems' . (($iduser != null) ? '&amp;id_user=' . $iduser : '') . '">' . get_string('solved_problems', 'lips') . '</a></li>';
    ...
    public function display_bad_solution($data, $lips) {
        ...
        $this->display_ace_form('aceSolution_' . $data->id . '_' . $data->row, '',
            $lips->coloration_language, 'readonly-notsizable');
    }
